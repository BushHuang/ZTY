...
class ConcatAdapterController implements NestedAdapterWrapper.Callback {
    ...
    public int getLocalAdapterPosition(RecyclerView.Adapter<? extends RecyclerView.ViewHolder> adapter, RecyclerView.ViewHolder viewHolder, int i) {
        ...
        if (iCountItemsBefore >= 0 && iCountItemsBefore < nestedAdapterWrapper.adapter.getItemCount()) {
            return nestedAdapterWrapper.adapter.findRelativeAdapterPositionIn(adapter, viewHolder, iCountItemsBefore);
        }
        throw new IllegalStateException("Detected inconsistent adapter updates. The local position of the view holder maps to " + iCountItemsBefore + " which is out of bounds for the adapter with size " + nestedAdapterWrapper.getCachedItemCount() + ".Make sure to immediately call notify methods in your adapter when you change the backing dataviewHolder:" + viewHolder + "adapter:" + adapter);
    }
    ...
    public boolean onFailedToRecycleView(RecyclerView.ViewHolder viewHolder) {
        NestedAdapterWrapper nestedAdapterWrapperRemove = this.mBinderLookup.remove(viewHolder);
        if (nestedAdapterWrapperRemove != null) {
            return nestedAdapterWrapperRemove.adapter.onFailedToRecycleView(viewHolder);
        }
        throw new IllegalStateException("Cannot find wrapper for " + viewHolder + ", seems like it is not bound by this adapter: " + this);
    }
    ...
    public void onViewRecycled(RecyclerView.ViewHolder viewHolder) {
        NestedAdapterWrapper nestedAdapterWrapperRemove = this.mBinderLookup.remove(viewHolder);
        if (nestedAdapterWrapperRemove != null) {
            nestedAdapterWrapperRemove.adapter.onViewRecycled(viewHolder);
            return;
        }
        throw new IllegalStateException("Cannot find wrapper for " + viewHolder + ", seems like it is not bound by this adapter: " + this);
    }
