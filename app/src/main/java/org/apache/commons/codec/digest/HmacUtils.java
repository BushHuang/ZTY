...
public final class HmacUtils {
    ...
    public static Mac getInitializedMac(String str, byte[] bArr) throws NoSuchAlgorithmException, InvalidKeyException {
        if (bArr == null) {
    ...
    public static boolean isAvailable(String str) throws NoSuchAlgorithmException {
        try {
            Mac.getInstance(str);
            return true;
    ...
    public static boolean isAvailable(HmacAlgorithms hmacAlgorithms) throws NoSuchAlgorithmException {
        try {
    ...
    public static Mac updateHmac(Mac mac, InputStream inputStream) throws IllegalStateException, IOException {
        mac.reset();
    ...
    public static Mac updateHmac(Mac mac, String str) throws IllegalStateException {
        mac.reset();
        mac.update(StringUtils.getBytesUtf8(str));
        return mac;
    }

    public static Mac updateHmac(Mac mac, byte[] bArr) throws IllegalStateException {
        mac.reset();
        mac.update(bArr);
        return mac;
    }
    ...
    public byte[] hmac(InputStream inputStream) throws IllegalStateException, IOException {
        byte[] bArr = new byte[1024];
        while (true) {
            int i = inputStream.read(bArr, 0, 1024);
            if (i <= -1) {
