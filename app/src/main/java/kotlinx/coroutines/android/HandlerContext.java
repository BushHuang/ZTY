...
public final class HandlerContext extends HandlerDispatcher implements Delay {
    ...
    private final void cancelOnRejection(CoroutineContext context, Runnable block) {
        ...
        Dispatchers.getIO().mo1709dispatch(context, block);
    }

    private static final void m1658invokeOnTimeout$lambda3(HandlerContext handlerContext, Runnable runnable) {
        ...
        if (this.handler.postDelayed(block, RangesKt.coerceAtMost(timeMillis, 4611686018427387903L))) {
            return new DisposableHandle() {
                ...
                public final void dispose() {
                    HandlerContext.m1658invokeOnTimeout$lambda3(this.f$0, block);
                }
            };
        }
        cancelOnRejection(context, block);
    ...
    public void mo1710scheduleResumeAfterDelay(long timeMillis, final CancellableContinuation<? super Unit> continuation) {
        ...
        if (this.handler.postDelayed(runnable, RangesKt.coerceAtMost(timeMillis, 4611686018427387903L))) {
            continuation.invokeOnCancellation(new Function1<Throwable, Unit>() {
                {
                    super(1);
                }
    ...
    public String toString() {
        ...
        String string = handlerContext.name;
        if (string == null) {
            string = handlerContext.handler.toString();
        }
        return handlerContext.invokeImmediately ? Intrinsics.stringPlus(string, ".immediate") : string;
    }
}
