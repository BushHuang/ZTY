...
public final class DebugProbesImpl {
    ...
    private static final SequenceNumberRefVolatile debugProbesImpl$SequenceNumberRefVolatile = new Object(0) {
    ...
    private static final AtomicLongFieldUpdater sequenceNumber$FU = AtomicLongFieldUpdater.newUpdater(SequenceNumberRefVolatile.class, "sequenceNumber");
    ...
    private static final class CoroutineOwner<T> implements Continuation<T>, CoroutineStackFrame {
        ...
        public CoroutineOwner(Continuation<? super T> continuation, DebugCoroutineInfo debugCoroutineInfoImpl, CoroutineStackFrame coroutineStackFrame) {
    ...
    private final <R> List<R> dumpCoroutinesInfoImpl(final Function2<? super CoroutineOwner<?>, ? super CoroutineContext, ? extends R> create) {
        ...
        try {
            if (INSTANCE.isInstalled$kotlinx_coroutines_core()) {
                return SequencesKt.toList(SequencesKt.mapNotNull(SequencesKt.sortedWith(CollectionsKt.asSequence(INSTANCE.getCapturedCoroutines()), new DebugProbesImpl$dumpCoroutinesInfoImpl$lambda12$$inlined$sortedBy$1()), new Function1<CoroutineOwner<?>, R>() {
                    {
                        super(1);
                    }
    ...
    private final void dumpCoroutinesSynchronized(PrintStream out) {
        ...
        try {
            ...
            for (CoroutineOwner coroutineOwner : SequencesKt.sortedWith(SequencesKt.filter(CollectionsKt.asSequence(INSTANCE.getCapturedCoroutines()), new Function1<CoroutineOwner<?>, Boolean>() {
                @Override
                ...
                List<StackTraceElement> listEnhanceStackTraceWithThreadDumpImpl = INSTANCE.enhanceStackTraceWithThreadDumpImpl(debugCoroutineInfoImpl.get_state(), debugCoroutineInfoImpl.lastObservedThread, listLastObservedStackTrace);
                out.print("\n\nCoroutine " + coroutineOwner.delegate + ", state: " + ((Intrinsics.areEqual(debugCoroutineInfoImpl.get_state(), "RUNNING") && listEnhanceStackTraceWithThreadDumpImpl == listLastObservedStackTrace) ? Intrinsics.stringPlus(debugCoroutineInfoImpl.get_state(), " (Last suspension stacktrace, not an actual stacktrace)") : debugCoroutineInfoImpl.get_state()));
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            ...
            objM228constructorimpl = Result.m228constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            objM228constructorimpl = Result.m228constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m234isFailureimpl(objM228constructorimpl)) {
            objM228constructorimpl = null;
        }
        StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) objM228constructorimpl;
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        objM228constructorimpl = Result.m228constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
        if (Result.m234isFailureimpl(objM228constructorimpl)) {
            objM228constructorimpl = null;
        }
        return (Function1) objM228constructorimpl;
    }
    ...
    private final void probeCoroutineCompleted(CoroutineOwner<?> owner) {
        ...
        CoroutineStackFrame coroutineStackFrameRealCaller = lastObservedFrame$kotlinx_coroutines_core == null ? null : realCaller(lastObservedFrame$kotlinx_coroutines_core);
    ...
    private final <T extends Throwable> List<StackTraceElement> sanitizeStackTrace(T throwable) {
        ...
        if (!sanitizeStackTraces) {
            ...
            while (i4 < i3) {
                int i5 = i4 + 1;
                arrayList.add(i4 == 0 ? StackTraceRecoveryKt.artificialFrame("Coroutine creation stacktrace") : stackTrace[i4 + i]);
                i4 = i5;
            }
            return arrayList;
        }
    ...
    private final void startWeakRefCleanerThread() {
        weakRefCleanerThread = ThreadsKt.thread((21 & 1) != 0, (21 & 2) != 0 ? false : true, (21 & 4) != 0 ? null : null, (21 & 8) != 0 ? null : "Coroutines Debugger Cleaner", (21 & 16) != 0 ? -1 : 0, new Function0<Unit>() {
            @Override
            public Unit invoke() {
                invoke2();
    ...
    private final void updateRunningState(CoroutineStackFrame frame, String state) {
        ...
        try {
            if (INSTANCE.isInstalled$kotlinx_coroutines_core()) {
                DebugCoroutineInfo debugCoroutineInfoImplRemove = callerInfoCache.remove(frame);
                if (debugCoroutineInfoImplRemove == null) {
                    CoroutineOwner<?> coroutineOwnerOwner = INSTANCE.owner(frame);
                    CoroutineStackFrame coroutineStackFrameRealCaller = null;
                    debugCoroutineInfoImplRemove = coroutineOwnerOwner == null ? null : coroutineOwnerOwner.info;
                    if (debugCoroutineInfoImplRemove == null) {
                        return;
                    }
                    ...
                    if (lastObservedFrame$kotlinx_coroutines_core != null) {
                        coroutineStackFrameRealCaller = INSTANCE.realCaller(lastObservedFrame$kotlinx_coroutines_core);
                    }
                    if (coroutineStackFrameRealCaller != null) {
                        callerInfoCache.remove(coroutineStackFrameRealCaller);
                    }
                ...
                callerInfoCache.put(coroutineStackFrameRealCaller2, debugCoroutineInfoImplRemove);
                Unit unit = Unit.INSTANCE;
            }
        } finally {
            lock.unlock();
    ...
    public final List<DebugCoroutineInfo> dumpCoroutinesInfo() {
        ...
        try {
            if (INSTANCE.isInstalled$kotlinx_coroutines_core()) {
                return SequencesKt.toList(SequencesKt.mapNotNull(SequencesKt.sortedWith(CollectionsKt.asSequence(INSTANCE.getCapturedCoroutines()), new DebugProbesImpl$dumpCoroutinesInfoImpl$lambda12$$inlined$sortedBy$1()), new Function1<CoroutineOwner<?>, DebugCoroutineInfo>() {
                    @Override
    ...
    public final List<DebuggerInfo> dumpDebuggerInfo() {
        ...
        try {
            if (INSTANCE.isInstalled$kotlinx_coroutines_core()) {
                return SequencesKt.toList(SequencesKt.mapNotNull(SequencesKt.sortedWith(CollectionsKt.asSequence(INSTANCE.getCapturedCoroutines()), new DebugProbesImpl$dumpCoroutinesInfoImpl$lambda12$$inlined$sortedBy$1()), new Function1<CoroutineOwner<?>, DebuggerInfo>() {
                    @Override
    ...
    public final <T> Continuation<T> probeCoroutineCreated$kotlinx_coroutines_core(Continuation<? super T> completion) {
        if (isInstalled$kotlinx_coroutines_core() && owner(completion) == null) {
            return createOwner(completion, enableCreationStackTraces ? toStackTraceFrame(sanitizeStackTrace(new Exception())) : (StackTraceFrame) null);
        }
        return completion;
    }
