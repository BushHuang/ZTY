...
public class ThreadSafeHeap<T extends ThreadSafeHeapNode & Comparable<? super T>> {
    ...
    public final T removeFirstIf(Function1<? super T, Boolean> predicate) {
        synchronized (this) {
            try {
                ...
                T t = predicate.invoke(threadSafeHeapNodeFirstImpl).booleanValue() ? (T) removeAtImpl(0) : (T) ((ThreadSafeHeapNode) null);
                InlineMarker.finallyStart(1);
                InlineMarker.finallyEnd(1);
                return t;
            } catch (Throwable th) {
    ...
    public final T removeFirstOrNull() {
        T t;
        synchronized (this) {
            t = get_size() > 0 ? (T) removeAtImpl(0) : (T) ((ThreadSafeHeapNode) null);
        }
        return t;
    }
}
