...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(DispatchedTask<? super T> dispatchedTask, int i) {
        ...
        if (coroutineDispatcher.isDispatchNeeded(coroutineContext)) {
            coroutineDispatcher.mo1709dispatch(coroutineContext, dispatchedTask);
        } else {
    ...
    public static final <T> void resume(DispatchedTask<? super T> dispatchedTask, Continuation<? super T> continuation, boolean z) {
        ...
        Object objM228constructorimpl = Result.m228constructorimpl(successfulResult$kotlinx_coroutines_core);
        if (!z) {
            continuation.resumeWith(objM228constructorimpl);
            return;
        }
        ...
        UndispatchedCoroutine<?> undispatchedCoroutineUpdateUndispatchedCompletion = objUpdateThreadContext != ThreadContextKt.NO_THREAD_ELEMENTS ? CoroutineContextKt.updateUndispatchedCompletion(continuation2, coroutineContext, objUpdateThreadContext) : (UndispatchedCoroutine) null;
        try {
            dispatchedContinuation.continuation.resumeWith(objM228constructorimpl);
    ...
    public static final void resumeWithStackTrace(Continuation<?> continuation, Throwable th) {
        ...
        continuation.resumeWith(Result.m228constructorimpl(ResultKt.createFailure(th)));
    }
