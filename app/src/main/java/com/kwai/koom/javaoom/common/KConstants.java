...
public class KConstants {
    ...
    public static class HeapThreshold {
        ...
        public static float getDefaultPercentRation() {
            int iMaxMemory = (int) (Runtime.getRuntime().maxMemory() / Bytes.MB);
            if (Debug.VERBOSE_LOG) {
                KLog.i("koom", "max mem " + iMaxMemory);
            }
            return iMaxMemory >= VM_512_DEVICE ? PERCENT_RATIO_IN_512_DEVICE : iMaxMemory >= VM_256_DEVICE ? PERCENT_RATIO_IN_256_DEVICE : iMaxMemory >= VM_128_DEVICE ? PERCENT_RATIO_IN_128_DEVICE : PERCENT_RATIO_IN_512_DEVICE;
        }
    }
