...
public abstract class FileUtil {
    ...
    public static class a implements Runnable {
        ...
        public void run() throws Throwable {
            ...
            try {
                try {
                    long length = this.f370a.length();
                    if (length > this.b) {
                        String canonicalPath = this.f370a.getCanonicalPath();
                        if (!this.f370a.delete()) {
                            HMSLog.e("FileUtil", "last file delete failed.");
                        }
                        randomAccessFile = new RandomAccessFile(new File(canonicalPath), "rw");
                    } else {
                        RandomAccessFile randomAccessFile2 = new RandomAccessFile(this.f370a, "rw");
                        try {
