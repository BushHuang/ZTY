...
public class TinkerInstaller {
    ...
    public static Tinker install(ApplicationLike applicationLike) {
        Tinker tinkerBuild = new Tinker.Builder(applicationLike.getApplication()).build();
        Tinker.create(tinkerBuild);
    ...
    public static Tinker install(ApplicationLike applicationLike, LoadReporter loadReporter, PatchReporter patchReporter, PatchListener patchListener, Class<? extends AbstractResultService> cls, AbstractPatch abstractPatch) {
        Tinker tinkerBuild = new Tinker.Builder(applicationLike.getApplication()).tinkerFlags(applicationLike.getTinkerFlags()).loadReport(loadReporter).listener(patchListener).patchReporter(patchReporter).tinkerLoadVerifyFlag(Boolean.valueOf(applicationLike.getTinkerLoadVerifyFlag())).build();
        Tinker.create(tinkerBuild);
