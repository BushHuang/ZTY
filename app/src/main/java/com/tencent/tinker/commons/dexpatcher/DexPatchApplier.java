...
public class DexPatchApplier {
    ...
    public void executeAndSaveTo(File file) throws Throwable {
    ...
    public void executeAndSaveTo(OutputStream outputStream) throws NoSuchAlgorithmException, IOException {
        ...
        if (CompareUtils.uArrCompare(bArrComputeSignature, oldDexSignature) != 0) {
            throw new IOException(String.format("old dex signature mismatch! expected: %s, actual: %s", Arrays.toString(bArrComputeSignature), Arrays.toString(oldDexSignature)));
        }
