...
public final class DexPatchFile {
    ...
    private void init() {
        ...
        if (CompareUtils.uArrCompare(byteArray, MAGIC) != 0) {
            throw new IllegalStateException("bad dex patch file magic: " + Arrays.toString(byteArray));
        }
        short s = this.buffer.readShort();
        this.version = s;
        if (CompareUtils.uCompare(s, (short) 2) != 0) {
