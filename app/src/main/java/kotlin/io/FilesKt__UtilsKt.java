...
class FilesKt__UtilsKt extends FilesKt__FileTreeWalkKt {
    public static final boolean copyRecursively(File file, File file2, boolean z, final Function2<? super File, ? super IOException, ? extends OnErrorAction> function2) {
        ...
        try {
            Iterator<File> it = FilesKt.walkTopDown(file).onFail(new Function2<File, IOException, Unit>() {
                {
                    super(2);
                }

                @Override
                public Unit invoke(File file3, IOException iOException) throws TerminateException {
                    invoke2(file3, iOException);
                    return Unit.INSTANCE;
                }

                public final void invoke2(File file3, IOException iOException) throws TerminateException {
            ...
            while (it.hasNext()) {
                ...
                if (next.exists()) {
                    ...
                    if (file3.exists() && (!next.isDirectory() || !file3.isDirectory())) {
                        if (z) {
                            if (file3.isDirectory()) {
                                z2 = FilesKt.deleteRecursively(file3) ? false : true;
                            } else {
                                if (!file3.delete()) {
                                }
                            }
                    ...
                    if (next.isDirectory()) {
                        file3.mkdirs();
                    } else if (FilesKt.copyTo$default(next, file3, z, 0, 4, null).length() != next.length() && function2.invoke(next, new IOException("Source file wasn't copied completely, length of destination file differs.")) == OnErrorAction.TERMINATE) {
                        return false;
                    }
    ...
    public static final File createTempDir(String str, String str2, File file) throws IOException {
        ...
        if (fileCreateTempFile.mkdir()) {
            Intrinsics.checkNotNullExpressionValue(fileCreateTempFile, "dir");
    ...
    private static final String toRelativeStringOrNull$FilesKt__UtilsKt(File file, File file2) {
        ...
        if (i < size2) {
            ...
            String str = File.separator;
            Intrinsics.checkNotNullExpressionValue(str, "separator");
            CollectionsKt.joinTo$default(CollectionsKt.drop(filePathComponentsNormalize$FilesKt__UtilsKt.getSegments(), i), sb, str, null, null, 0, null, null, 124, null);
        }
        return sb.toString();
    }
}
