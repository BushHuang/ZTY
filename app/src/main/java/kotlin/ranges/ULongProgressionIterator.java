...
final class ULongProgressionIterator extends ULongIterator {
    ...
    private ULongProgressionIterator(long j, long j2, long j3) {
        ...
        boolean z = true;
        int iUlongCompare = UnsignedKt.ulongCompare(j, j2);
        if (j3 <= 0 ? iUlongCompare < 0 : iUlongCompare > 0) {
            z = false;
        }
        this.hasNext = z;
        this.step = ULong.m400constructorimpl(j3);
    ...
    public long mo469nextULongsVKNKU() {
        ...
        if (j != this.finalElement) {
            this.next = ULong.m400constructorimpl(this.step + j);
        } else {
            if (!this.hasNext) {
