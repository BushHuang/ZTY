...
final class RunSuspend implements Continuation<Unit> {
    ...
    public final void await() {
        synchronized (this) {
            while (true) {
                Result<Unit> resultM1415getResultxLWZpok = m1415getResultxLWZpok();
                if (resultM1415getResultxLWZpok == null) {
                    wait();
                } else {
                    ResultKt.throwOnFailure(resultM1415getResultxLWZpok.getValue());
                }
            }
        }
    }
    ...
    public final Result<Unit> m1415getResultxLWZpok() {
        return this.result;
    }
    ...
    public void resumeWith(Object result) {
        synchronized (this) {
            this.result = Result.m227boximpl(result);
            notifyAll();
