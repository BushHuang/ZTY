...
public final class RegexKt {
    ...
    private static final <T extends Enum<T> & FlagEnum> Set<T> fromInt(final int i) {
        Intrinsics.reifiedOperationMarker(4, "T");
        EnumSet enumSetAllOf = EnumSet.allOf(Enum.class);
        EnumSet enumSet = enumSetAllOf;
        Intrinsics.checkNotNullExpressionValue(enumSet, "");
        Intrinsics.needClassReification();
        CollectionsKt.retainAll(enumSet, new Function1<T, Boolean>() {
            {
                super(1);
            }
    ...
    private static final IntRange range(java.util.regex.MatchResult matchResult, int i) {
